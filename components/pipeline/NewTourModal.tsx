...
export default function NewTourModal({ onClose, onCreate }: Props) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 300, background: "rgba(10,25,40,.65)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: "var(--surface)", borderRadius: 16, boxShadow: "0 24px 80px rgba(0,0,0,.3)", width: "100%", maxWidth: 520, maxHeight: "92vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 24px", borderBottom: "1.5px solid var(--surface-3)", background: BRAND.navy }}>
          <span style={{ fontSize: 15, fontWeight: 400, color: "#fff", fontFamily: "'Fjalla One', Georgia, sans-serif", letterSpacing: 0.3 }}>New Tour</span>
        ...
        <form onSubmit={handleSubmit} style={{ padding: 24, overflowY: "auto", flex: 1, display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            <label style={{ fontSize: 11, fontWeight: 700, color: "var(--muted-2)", textTransform: "uppercase", letterSpacing: 0.8 }}>Tour Name *</label>
            <input style={inp} value={form.name} onChange={e => f({ name: e.target.value })} placeholder="Westridge HS — NYC Spring Musical" autoFocus />
          </div>

          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            <label style={{ fontSize: 11, fontWeight: 700, color: "var(--muted-2)", textTransform: "uppercase", letterSpacing: 0.8 }}>School *</label>
            <input style={inp} value={form.school} onChange={e => f({ school: e.target.value })} placeholder="Westridge High School" />
          </div>

          <div style={{ display: "flex", gap: 12 }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 4, flex: 1 }}>
              <label style={{ fontSize: 11, fontWeight: 700, color: "var(--muted-2)", textTransform: "uppercase", letterSpacing: 0.8 }}>Destination</label>
              <input style={inp} value={form.destination} onChange={e => f({ destination: e.target.value })} placeholder="New York, NY" />
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 4, flex: 1 }}>
              <label style={{ fontSize: 11, fontWeight: 700, color: "var(--muted-2)", textTransform: "uppercase", letterSpacing: 0.8 }}>Dates</label>
          ...
          <div style={{ display: "flex", gap: 12 }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 4, flex: 1 }}>
              <label style={{ fontSize: 11, fontWeight: 700, color: "var(--muted-2)", textTransform: "uppercase", letterSpacing: 0.8 }}>Status</label>
              <select style={{ ...inp, background: "var(--surface)" }} value={form.status} onChange={e => f({ status: e.target.value })}>
                {STATUSES.map(s => <option key={s.id} value={s.id}>{s.label}</option>)}
              </select>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 4, flex: 1 }}>
              <label style={{ fontSize: 11, fontWeight: 700, color: "var(--muted-2)", textTransform: "uppercase", letterSpacing: 0.8 }}>Transport</label>
              <select style={{ ...inp, background: "var(--surface)" }} value={form.transport_type} onChange={e => f({ transport_type: e.target.value })}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            <label style={{ fontSize: 11, fontWeight: 700, color: "var(--muted-2)", textTransform: "uppercase", letterSpacing: 0.8 }}>Participant Personas</label>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {PERSONAS.map(p => {
                ...
                return (
                  <div key={p.key} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <span style={{ width: 84, flexShrink: 0, fontSize: 12, fontWeight: 700, color: on ? "var(--text)" : "var(--muted-2)" }}>{p.default}</span>
                    <input
          ...
          {error && (
            <div style={{ background: "var(--red-bg)", color: "var(--red-text)", borderRadius: 8, padding: "9px 14px", fontSize: 13 }}>{error}</div>
          )}

          <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
            <button type="button" onClick={onClose} style={{ flex: 1, background: "var(--surface-3)", color: "var(--muted)", border: "1.5px solid var(--border)", borderRadius: 8, padding: "9px 0", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
              Cancel
            </button>

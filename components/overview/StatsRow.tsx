      ...
      <div className="stats-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 12 }}>
        {STATUSES.map(st => (
          ...
          >
            ...
            <div style={{ fontSize: 32, fontWeight: 400, color: "var(--ink)", lineHeight: 1, fontFamily: "'Fjalla One', Georgia, sans-serif" }}>
              {counts[st.id]}
            </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
            <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 8, background: "var(--green-bg-soft)", border: "1px solid var(--green-border)", borderRadius: 9, padding: "6px 10px" }}>
              <span style={{ fontSize: 10.5, fontWeight: 700, color: "var(--green-text)", textTransform: "uppercase", letterSpacing: 0.5 }}>Received</span>
              <span style={{ fontSize: 17, fontWeight: 400, color: "#16a34a", fontFamily: "'Fjalla One', Georgia, sans-serif", letterSpacing: "0.03em" }}>{usd(received)}</span>
            </div>
            <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 8, background: "var(--amber-bg-soft)", border: "1px solid var(--amber-border)", borderRadius: 9, padding: "6px 10px" }}>
              <span style={{ fontSize: 10.5, fontWeight: 700, color: "var(--amber-text)", textTransform: "uppercase", letterSpacing: 0.5 }}>Pending</span>
              <span style={{ fontSize: 17, fontWeight: 400, color: "#d97706", fontFamily: "'Fjalla One', Georgia, sans-serif", letterSpacing: "0.03em" }}>{usd(pending)}</span>
            </div>
          </div>
        </div>
        ...
        <div
          style={{
            background: "var(--surface-2)", border: "1.5px dashed var(--border)", borderRadius: 14,
            padding: "14px 18px", opacity: 0.85,
          }}
        >
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 6, marginBottom: 6 }}>
            <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 11, fontWeight: 700, color: "var(--muted-2)", textTransform: "uppercase", letterSpacing: 0.6 }}>
              <Link2 size={13} />Financial Integrations
            </span>
            <span style={{ fontSize: 8.5, fontWeight: 700, color: "var(--muted-2)", background: "var(--border-soft)", borderRadius: 20, padding: "2px 7px", textTransform: "uppercase", letterSpacing: 0.5, whiteSpace: "nowrap" }}>
              Coming Soon
            </span>
          </div>
          <div style={{ fontSize: 26, fontWeight: 400, color: "var(--muted-3)", lineHeight: 1, fontFamily: "'Fjalla One', Georgia, sans-serif" }}>
            —
          </div>
          <div style={{ fontSize: 11, color: "var(--muted-3)", marginTop: 5 }}>
...
function ComingSoonTile({ label, hint }: { label: string; hint: string }) {
  return (
    ...
    >
      ...
      <div style={{ fontSize: 11, color: "var(--muted-3)", marginTop: 5 }}>{hint}</div>
    </div>
  );
}

-- August 2026 follow-ups (Seth, after Amy's batch shipped)
-- 1. agenda_items.custom_type_label: the write-in activity type name.
-- 2. tours.trip_info_row_order: host-chosen order of Trip Information rows.
-- 3. get_shared_tour: strict superset (+ trip_info_row_order).

alter table public.agenda_items add column if not exists custom_type_label text;
alter table public.tours add column if not exists trip_info_row_order text[] not null default '{}';
